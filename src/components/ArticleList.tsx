...
export function ArticleList({
  ...
  duplicatesById: Map<number, DuplicateRef[]>;
}) {
    ...
    <div>
      <div className="hidden md:block">
        <Table>
          ...
          <TableBody>
            {articles.map((a) => (
              <ArticleCard key={a.id} article={a} analysis={analysesById.get(a.id)} duplicates={duplicatesById.get(a.id) ?? []} />
            ))}
          </TableBody>
        </Table>
      </div>
      <div className="flex flex-col gap-2 py-2 md:hidden">
        {articles.map((a) => (
          <ArticleCardMobile key={a.id} article={a} analysis={analysesById.get(a.id)} duplicates={duplicatesById.get(a.id) ?? []} />
        ))}
      </div>
