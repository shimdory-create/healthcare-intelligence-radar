...
export async function GET(req: NextRequest) {
  ...
  // outside the normal collection cron. Each step is isolated in its own try/catch so one
  // source's transient failure doesn't blank out the whole diagnostic response.
  ...
  try {
    ...
    result.khidi = { error: err instanceof Error ? err.message : String(err) };
  }
  ...
  try {
    ...
    result.joongang = { error: err instanceof Error ? err.message : String(err) };
  }
