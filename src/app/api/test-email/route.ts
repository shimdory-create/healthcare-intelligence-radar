...
export async function GET(req: NextRequest) {
  ...
  await sendDigestEmail(articles, counts, formatKstDate(collectedDate), analysesById);
  return NextResponse.json({ sent: true, collectedDate, total: counts.total, analyzed: analyses.length });
}
